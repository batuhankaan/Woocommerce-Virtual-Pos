...
class EticSoft_POSNET
{
    ...
    public function pay($tr)
    {
        ...
        $orderid = substr(str_pad("ETICSOFT_" . $tr->id_cart, 24, "0", STR_PAD_LEFT), 0, 24);
        $tr->boid = $orderid;
    ...
    public function tdForm($tr)
    {
        ...
        $xid = substr(str_pad('ETICSOFT' . $tr->id_transaction, 20, "0", STR_PAD_LEFT), 0, 20);
        $instnumber = $tr->installment;
        $amount = (int) ($tr->total_pay * 100);
        $currencycode = $tr->currency_code == "TRY" ? "TL" : $tr->currency_code;
    ...
    public function tdValidate($tr)
    {
        ...
        $session = new Session();
		$session->xid = substr(str_pad('ETICSOFT' . $tr->id_transaction, 20, "0", STR_PAD_LEFT), 0, 20);// i�lemin id si
